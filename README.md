...
- 多平台构建与发布已配置 GitHub Actions（`.github/workflows/release.yml`）：推送 `v*` 标签自动发布，或手动触发并指定 `tag_name`；当前矩阵覆盖 darwin-arm64、linux-x64/arm64、win32-x64（darwin-x64 / win32-arm64 需自托管 runner）。
