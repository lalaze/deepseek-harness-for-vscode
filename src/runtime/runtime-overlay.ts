...
`
  // Older builds persisted a custom provider but discarded its custom model
  // ID. In that state, omitting this row lets Harness recover the valid pair
  // already stored in settings.yaml instead of combining the provider with
  // the extension's built-in DeepSeek fallback.
  const defaultModelRow = configuration.modelSelectionConfigured ? `- id: agent-default-model
  config:
...
${defaultModelRow}- id: agent-presets
  config:
