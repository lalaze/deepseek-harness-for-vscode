...
- 修复自定义视觉模型发送图片时被误判为纯文本模型的问题：连接配置会为可识别的 `vision`、`image`、`vl`、`multimodal` 模型 ID 声明图片输入能力，并自动修复已有配置；名称不明显的视觉模型可在连接设置中显式填写。
- 自定义 API 新增“支持的最高思考强度”配置，可选择低、高或最高；消息编辑器会据此展示“关闭”及不高于上限的档位，已有自定义 API 默认保持完整四档。
- 连接设置中的内置 DeepSeek 来源现可直接编辑并保存兼容网关 Base URL，不再被前端禁用或被服务层限制为 `api.deepseek.com`；首次配置横幅会打开完整连接设置，不再跳过自定义地址而直接弹出官方 Key 输入框。需要自定义模型 ID 或多个端点时仍可使用独立中转来源。
